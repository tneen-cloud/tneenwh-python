...
Python client for the **[TNEENWH](https://pypi.org/project/tneenwh/)** WhatsApp HTTP API — authentication, OTP signup, sessions, outbound messaging, webhooks, media, groups (when the server exposes them), and admin helpers.
...
7. [Assistant (dashboard AI)](#assistant-dashboard-ai)
8. [Sessions (panel JWT)](#sessions-panel-jwt)
9. [Messaging (default session)](#messaging-default-session)
10. [Channel object (`session(...)`)](#channel-object-session)
11. [Sub-API (`x-api-key`)](#sub-api-x-api-key)
12. [Groups](#groups)
13. [OTP helper (no HTTP)](#otp-helper-no-http)
14. [Not available on stock HTTP mapping](#not-available-on-stock-http-mapping)
15. [Errors](#errors)
